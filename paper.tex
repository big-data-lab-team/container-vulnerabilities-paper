...
d,bids/broccoli:v1.0.0,203,203,117,
...
ab,mcin/qeeg:latest,121,121,142,
ac,camarasu/creaphase:0.3,148,148,186,
ad,bigdatalabteam/hcp-prefreesurfer:exec-centos7.freesurferbuild-centos4-latest:latest,257,266,157,
ae,mcin/docker-fsl:latest,178,178,201,
af,gkiar/onevoxel:v0.3.0rc2,5,2,2,
ag,bigdatalabteam/hcp-prefreesurfer:exec-centos7-fslbuild-centos5-latest:latest,257,266,157,
ah,bt5e/ants:latest,592,600,336,
...
al,mcin/ica-aroma:latest,178,178,202,
...
totalling
44 container images: 26 from BIDS apps, and 18 from Boutiques. At the time of the study, BIDS apps had 27 images,
...
and 6 Singularity images. \tristan{Check the numbers: that makes 24 Boutiques images, not 18.}
\change{Boutiques images are total 18 which is 12(docker)+6(Singularity)}.

\subsection{Image Scanners}

We used four popular image scanners, namely Anchore,
Vuls and Clair, supporting Docker, and
Singularity Container Tools --- Stools, supporting Singularity. 
...
container clusters. Clair has a client-server architecture, in which the
server scans Docker images layer by layer and maintains a database of
vulnerabilities. We used Clair through
...
Table~\ref{table:databases} shows the vulnerabilities databases used by
each scanner by OS distribution. Scanners refer to two types of
vulnerability databases. The first one is the Open Vulnerability and
...
CVE Tracker, or Red Hat Security Data. OS distributions often assign a
...
databases for all distributions except Alpine, since Alpine is not
available in OVAL. On the contrary, Clair exclusively refers to
distribution-specific databases. Anchore uses OVAL only for CentOS
\tristan{any idea why?}.\change{Through our scanning experiments I learned that
Oval Centos database was not listing all vulnerabilities and Anchore team already
realized that. In their recent releeases Centos OVAL is deprecated and they started 
referring to Red Hat Enterprise Linux (RHEL) data} It is also worth noting that there is no vulnerability data
...
distributions cannot be scanned with Vuls \tristan{why doesn't vuls use
...
refers to the NVD database for getting vulnerability metadata \tristan{why is NVD not in the table?}.
\change{NVD is used by all scanners for all Operating systems for getting
generic vulnerability metadata so if we write in table it will be
repetitive info}.
...
\change{We updated images on 2019-11-05}.
...
packages required for the system to function, \tristan{can you give an
example?}\change{such as \texttt{coreutils} and \texttt{bash}}, and with all the dependencies of the required packages,
retrieved using
...
\tristan{Add info in README, clean up code wherever needed}, installs
\reprozip in the image to minify, runs an analysis to collect a \reprozip
trace, and finally deletes all unnecessary packages. We had used the
\href{https://github.com/ReproNim/neurodocker}{Neurodocker} tool initially, but it did not have any impact on the detected
vulnerabilities because it was removing unused files from the file system
directly instead of using the package manager \tristan{Please check this last sentence}.
\change{correct}.
...
Anchore in container images. \textbf{(A)} Number of vulnerabilities by
container image and severity, showing hundreds of detected vulnerabilities
per image (mean: 468). \textbf{(B)} Effect of image minification and
package update on 5 container images, showing that both techniques are
complementary \tristan{image f has 0 vulnerabilities after update and minification, is it really the case?
it is also weird that minification before update has almost no effect, but it removes all the remaining vulnerabilities after update}.
	\change{image f is bids/ndmg:latest and it have 329 vulnerabilities after update
	and minification together. However, minification alone does not have much effect and it have 930
	vulnerabilities after minification} \textbf{(C)} Number of vulnerabilities by number of
...
An important amount of vulnerabilities were found in the tested container
 ...
 vulnerabilities was observed \tristan{note to self: this isn't true for Alpine}.
...
reach EOL}. \change{3 out of these missing 6 images retured an error while updating which
...
The vulnerabilities detected by Anchore only (region \textbf{(1)} in
Fig~\ref{fig:venn}) are all in the development package (headers) of the C
library (\texttt{linux-libc-dev} in Ubuntu). \tristan{not all in fact... I
wasn't sure how to interpret all these differences (see last section in the
notebook), let's discuss it during a meeting.}
...
% 	\end{table*}
	...
	\tristan{add Centos images to the analysis using the notebook}}
\end{figure}
...
for 5 years, and Debian stable releases are maintained for 3 years. Commercial
