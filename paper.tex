...
Also, Singularity came up with Stools \href{https://github.com/singularityhub/stools}{(https://github.com/singularityhub/stools)}, which can be used for scanning Singularity images
...
vulnerabilities. The next section of this paper provides the results, where we
...
we faced while conducting these experiments. Finally, we conclude this paper by proposing some
particular rules that can be followed to secure container images for use in clusters.

\section{Materials and Methods}

\change{This study involves several image scanning experiments. To begin with,
...
out which packages we need for the pipeline to function properly. Here, we use
...
~\cite{debtree}.
Ultimately, we keep these packages and remove remaining unnecessary packages. We use a bash script
...
vulnerabilities.
\tristan{Probably not really at random... 
...
We did this experiment only on five images because the reduction process takes
several hours.\tristan{This is not a good argument, it is vague, and it is not complete. Size reduction also requires 
