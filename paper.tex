...
\change{There are two types of vulnerability databases that are referred
by the scanners in these experiments. First one is Open Vulnerability and Assessment Language (OVAL)
database, which is an international open standard and a common means of determining the existence of
vulnerabilities in softwares. OVAL supports various OS distributions e.g. Ubuntu, Debian, Centos, etc.
Second type of databases used by scanners are vulnerability databases from specific OS vendors e.g. Alpine-SecDB
from Alpine,
Debian Bug Tracker from Debian, Ubuntu CVE Tracker from Ubuntu, etc., where they
keep a track of their OS-specific vulnerabilities by assigning a status.
%In particular, there is a discrepancy in the way Anchore and Clair
%interpret Ubuntu CVE Tracker.
Out of four distributions that are present in our images Vuls uses
OVAL database for Debian, Ubuntu, Centos and refers to Alpine-SecDB
database for Alpine as this distribution is currently not supported by OVAL.
Clair and Anchore mostly refers to vulnerability databases provided by specific OS
vendors except in one case, where Anchore refers to OVAL i.e. Centos.
