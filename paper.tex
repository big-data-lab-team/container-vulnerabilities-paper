...
Singularity Container Tools --- Stools~\cite{stools}, supporting Singularity. 
...
\tristan{any idea why?}. It is also worth noting that there is no vulnerability data
...
refers NVD database for getting vulnerability metadata \tristan{why is NVD not in the table?}.
...
example?}, and with all the dependencies of the required packages,
retrieved using
...
it is also weird that minification before update has almost no effect, but it removes all the remaining vulnerabilities after update}.
 \textbf{(C)} Number of vulnerabilities by number of
...
38 images involved in this study takes about \todo{y} hours) that
