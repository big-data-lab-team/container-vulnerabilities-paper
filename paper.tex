...
\begin{keywords}
Containers; Docker; Singularity; Security; Vulnerabilities; Image Scanning; Outdated packages; Shrinking Images; Neuroimaging.
\end{keywords}
...
popular. This is due to them being lightweight, flexible, and resource-efficient.
Additionally, they are capable of providing fast development cycles and continuous delivery with the
reduction of infrastructure cost~\cite{martin2018docker}. Containers are heavily utilised in various scientific fields,
in particular, to run big data analysis. OS-level virtualization gained a lot of interest when Docker~\cite{docker19} was
introduced in 2013. However, Docker is not supported by High Performance Computing (HPC)
environments, mainly due to the fact that Docker user can act as superuser inside the container.
As a result, Singularity~\cite{singularity19} was developed
for scientific applications by keeping the focus on HPC environments. In addition, Singularity also solves some other security issues
related to the architecture of Docker. It is used in many clusters including Compute Canada.
...
Inspite of their popularity, containers tightly
integrate with the host due to the sharing of the kernel, 
mounted file systems, and devices, which raises security
concerns.
In fact, in relation to other virtualization techniques, they share three main security threats~\cite{gantikow2016providing}:
...
\textit{Denial-of-Service} One container may end up consuming all the resources of the
...
In order to mitigate these threats, containers use some features of the Linux kernel
such as \textit{control groups, namespaces}, etc. For instance, \textit{control groups}
are used to restrict the amount of resources that a container can use. Nonetheless, container's
security is still one of the concerning issues.
...
%of containers is still the most concerning issue. 
According to a 2015 Forrester survey, security is the
biggest concern when deploying containers~\cite{bettini2015vulnerability}.
...
%Above mentioned threats are not the only security issues that containers may suffer.
%Other security issues are related to the architecture of Docker but have been 
...
%in neuroscience.
While containers still suffer general security problems that may affect any application using
them, our particular focus in this research is the security of container images.
In this research work, we aim to study and analyse the security of container images.
Although this work, is of general interest and can be applied on any field, we focus in this
paper on neuroimaging. This is due to the fact that containers are particulary popular in
scientific computing and heavily utiilised in neuroscience, which makes it a good candidate
for this research.
This popularity of containers in neuroscience leads us to the following
primary research question:

\textit{RQ1: What is the current security state of
neuroscience container images that are deployed on HPC clusters?}

In fact, answering this question leads us to further questions as described below. 
To answer our primary research question, we scan neuroscience images. The purpose
of this scanning process is to collect information about the container images
and then investigate these findings against security vulnerabilities.
%we scan images that are used on Compute
...
%report to summarize scanning results. 
It is broadly categorized into
two types: \textit{Static} and \textit{Dynamic} scanning. The former
involves extracting information of an image without running that image into a container,
whereas the latter involves running a container with the target image.
While scanning may provide different information, here, we focus mainly on security vulnerability.
These types of vulnerabilities may be caused by an exploitable weakness in developer's software.
Such exploitation may result in serious attacks such as gaining unauthorized access on the target
computer, deletion of mounted files, etc. In this work, we target the detection of vulnerabilities
that are present in neuroscience container images. It is further important to answer following three 
secondary questions:
...
\textit{RQ2:} Can vulnerabilities be reduced by updating the images?

\textit{RQ3:} Can vulnerabilities be reduced by shrinking the size of the images?
...
of materials and tools, which we utilise throughout our experiments. We also present
different image scanners used for vulnerability reporting in container images.
The following section of this paper provides our approach and methodology, where we
mainly examine the effect of image update and image reduction for the purpose
of reducing vulnerabilities. Afterwards, we provide the results of our experiments,
where we indicate a number of detected vulnerabilities in container images used for
data analysis on shared HPC clusters. The last two sections provide a conclusion of this
work following by a summary and an indication of our future work.
...
%Among the 27 BIDS apps \href{https://bids-apps.neuroimaging.io/apps/}
...
%Therefore, package manager, which is a executable file,
...
%Vuls support both static as well as dynamic
...
%Further, Vuls offer a fast scan, fast root scan,
%and deep scan mode.
...
In our experiments, we use a Vuls Docker image having version 0.9.0. %with fast scan mode.
...
%is a registry to store images privately, uses Clair internally as the security engine. 
%However, Clair does not 
...
%So, to remove
%that drawback, another tool, Clair Scanner can be used. 
...
Singularity images.
%Stools combines CoreOS's Clair with Continuous Integration (CI)
%for scanning Singularity images for vulnerabilities.
%Initially, Clair was intended to scan Docker images only. This tool does not fit well
...
%	  \multirow{2}{*}{\bfseries Image Labels} & 
%	  \multirow{2}{*}{\bfseries Image Names} & 
%	  \multirow{2}{*}{\bfseries OS} & 
%	  \multicolumn{4}{c|}{\bfseries Vulnerabilities} \\\cline{2-5} 
