...
vulnerability data related to Ubuntu \tristan{You need to be accurate on these sources, this is not ``for instance". 
...
\todo{add Docker image name}. Vuls gets data from the Open Vulnerability
and Assessment Language (OVAL) database \tristan{Does it contain
vulnerabilities for all distributions?} \tristan{The Vuls website says that it uses multiple vulnerability databases: https://vuls.io/docs/en/main-features.html Are you sure 
that you only used OVAL and if so, why?}. It is worth noting that there is
...
scanner maintains a Docker image with the up-to-date Ubuntu CVE tracker
database, used for the analysis \tristan{Please mention the vulnerability sources for all distributions}.
...
and we follow the steps mentioned in
...
is used to make applications reproducible \tristan{No it doesn't make applications reproducible}. This tool traces operating system calls used by the
...
So, a list of packages that are needed for the application re-execution is determined. \tristan{How did you run the applications? Which command lines, why, etc} Following that, the list
...
~\cite{debtree} \tristan{does debtree work in centos? if not, how do you minimize centos images?}.
...
the scanners and the overlap between them. \tristan{According to our discussion, this is only for Ubuntu. This should be clarified.}
