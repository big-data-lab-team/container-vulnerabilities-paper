...
\begin{comment}
This work involves three experiments.
First experiment involves scanning
...
from where we took containerized applications for our study. Later, we discuss 
various Linux distributions and their releases. Lastly, this section
provides a description of the scanning tools used.
...
application to function properly. The functioning of an application in an isolated environment
is called a container. Containers directly run on the Host machine's kernel so they are lightweight.
...
In our experiments, we used $2.5.2$ Singularity version. 
\begin{comment}
...
Therefore we are left with total 18 Boutiques images which includes 6
...
of different applications through containerization.
At the time of this study, there were 27 BIDS apps available and all are using different Docker images.
Out of these 27 images, we used 26 images for our experiments because QAP bids app was not available
on Docker hub.

So we have total 38 Docker images from both BIDS Apps and Boutiques.
...
In Ubuntu launchpad database, for a given CVE, there is entry for all ubuntu releases.
...
\textbf{ignored:} The package (inside the given release), while related to the
		...
		is an extended security support for this release then this
...
\textbf{pending:} The package (inside the given release) is vulnerable, and
		  ...
		  pushlished. The "version" indicates the package version 
...
is a registery to store images privately, uses Clair internally as the security engine. However, Clair does not 
...
to scan Singularity images because of the different image formats. Docker images
...
In this section, we present and compare scanning results of three scanners. 
...
lightweight distribution, have least number of vulnerabilities, whereas Ubuntu contains highest number of
vulnerabilities. Debian and Cnetos contain higher number of critical vulnerabilities than Ubuntu and Alpine.
...
Table~\ref{table1} compares results of scanning images with the three scanners.
...
libavlibavlibavlibavlibavlibavlibav
...
For example, we scanned an image having Trusty release in it and Vuls is reporting CVE-2015-2305
...
For example CVE-2017-9994 vulnerability in libav package is ignored in Trusty LTS release due end-of-life.
so it is not reported by Anchore. Whereas, this vulnerability has status as "DNE" in Trusty ESM
release so Clair Scanner also do not report it. As Ubuntu 14.04 LTS release is present inside the images still
