...
the server Docker Daemon also known as Docker engine.
...
out which packages we need for the pipeline to fucntion properly. Here, we use Reprozip
\href{https://github.com/VIDA-NYU/reprozip}{https://github.com/VIDA-NYU/reprozip}. It is a tool that
is used to make applications reproducible~\cite{ChirigatiRSF16}. It traces operating system calls used by the
...
So, we got this list of packages from Reprozip that are needed for the application execution. 
...
(\href{http://manpages.ubuntu.com/manpages/xenial/man1/debtree.1.html}{http://manpages.ubuntu.com/manpages/xenial/man1/debtree.1.html}).
Ultimately, we kept these packages and removed remaining unnecessary packages. We used bash script
...
the number of vulnerabilities by significant number.
